...
function generateId(){
  ...
  for(let tries = 0; tries < 100; tries++){
    let name = [oneof(names),oneof(names)].join('-');
  ...
  throw new Error('Ran out of random names.')
}
...
let collection = news().quarantine.map(i=>{i.type = 'quarantine'; return i}).concat(news().research.map(i=>{i.type = 'research'; return i})).sort()

var turndownService = new TurndownService({linkStyle: 'referenced'})
